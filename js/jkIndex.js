(function(window){
    ...
    window.onscroll = function() {
        ...
        if (scrollTop >= 150) {
            $('jk_top_nav').style.display = "block";
        } else {
            $('jk_top_nav').style.display = "none";
        }

        //瀑布流加载新图片的条件
        if (jkCheckWillLoadImage()) {
            // setTimeout(() => {
                jkAutoCreateImg();
                jkWaterFull('jk_dom_pull','box');
            // }, 200);
 ...
 *
 */
function jkBannerAutoPlay() {
    // document.getElementsByTagName
    // 获取所有的li标签
    var allLis = $('jk_slider_banner').getElementsByTagName('li');
    // var allLis = $('jk_slider_banner');
    ...
    setInterval(function() {
        ...
        for(var i = 0;i < allLis.length; i++) {
            var singerLi = allLis[i];
            buffer(singerLi,{opacity:0},null);
        }
        buffer(allLis[index],{opacity:1},null);
 ...
 *
 */
function jkSwitchTabAction() {
    //先获取li
    var allLis = $('jk_tab_header').getElementsByTagName('li');
    //获取底部的内容切换试图
    var allJKDoms = $('jk_tab_content').getElementsByClassName('jk-dom');
    // console.log(allJKDoms);
    ...
    for (var i = 0 ; i < allLis.length; i ++ ){
        ...
        (function(k) {
            li.onmousedown = function() {
                allLis[lastSelectedIndex].className = "";
                allJKDoms[lastSelectedIndex].style.display = 'none';
                this.className = 'jk-current';
                allJKDoms[k].style.display = 'block';
                lastSelectedIndex = k;
...
function jkAutoCreateImg() {
    ...
    for(var i = 0; i < 30; i++) {
        ...
        // 1.2.3 拼接
        str += htmlStr;
        $("jk_dom_pull").innerHTML = str;

        
    }

    //绑定时间
    var wrapBox = $('jk_dom_pull').getElementsByClassName('box');
    var wrapPic = $('jk_dom_pull').getElementsByClassName('pic');

    console.log('++++'+wrapBox.length);
    console.log("----"+wrapPic.length);

    console.log(wrapBox[0].childNodes);
